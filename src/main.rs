...
use std::{collections::HashSet, ffi::OsStr, fs, io::{stdout, Write}, path::{Path, PathBuf}, sync::Arc};
...
impl WatcherContext {
    fn handle_vfs_event(&mut self, event: VfsEvent) {
        ...
        if let Some(file_name) = path.file_name() && file_name.to_string_lossy().ends_with(".model.json") { return }
        ...
        if path.is_file() {
            ...
            } else if let Some((luaurc_path, _)) = &self.luaurc && &path == luaurc_path {
            ...
            CreateFileDependencies::True(referent_path) => {
                ...
                if let Some(referent_path) = referent_path {
                    for dependant in dependants.clone() {
                        if referent_path == dependant.as_ref() { continue }
    ...
    fn luaurc_update(&mut self, luaurc_path: PathBuf) {
        let new_aliases = guarded_unwrap!(
            fs::read_to_string(&luaurc_path).map(Aliases::new), return
        );
        ...
        let new_luaurc = Luaurc { aliases: Aliases(new_aliases.clone()), dependants: luaurc.dependants };
    ...
    fn prune_dependencies(&mut self, deleted_path: &Path) {
        let keys_to_prune_from_dependencies = self.dependencies.left_to_right.keys()
            .filter_map(|key| {
                if key.starts_with(deleted_path) { Some(key.to_path_buf()) }
                else { None }
            })
            .collect::<Vec<_>>();
        ...
        if let Some((_, luaurc)) = self.luaurc.as_mut() {
             let keys_to_prune_from_luaurc_dependants = luaurc.dependants.right_to_left.keys()
                .filter_map(|key| {
                    if key.starts_with(deleted_path) { Some(key.to_path_buf()) }
                    else { None }
                })
    ...
    fn new(
        ...
        Self {
            ...
            luaurc: luaurc_path
                .map(|luaurc_path| {
                    let read_to_string =
                        fs::read_to_string(&luaurc_path);

                    (luaurc_path.clone(), read_to_string
                        .map(Luaurc::new)
                        .unwrap_or_default())
                })
        }
...
impl FindFirstChild for Path {
        ...
        F: FnMut(&PathBuf) -> bool,
    {
        for entry in fs::read_dir(self).ok()? {
            let entry = entry.ok()?;
            let path = entry.path();
            if predicate(&path) { return Some(path) }
        }
...
impl LuaurcStatus {
    fn as_option(&self) -> Option<&PathBuf> {
        match self {
            Self::Some(luaurc_path) |
            Self::AutoSome(luaurc_path) => Some(luaurc_path),
...
fn resolve_luaurc_path(input_dir: &PathBuf, luaurc_path: Option<PathBuf>) -> Result<LuaurcStatus, String> {
    if let Some(luaurc_path) = luaurc_path {
        match luaurc_path.canonicalize() {
            ...
            Err(_) => return Err(format!("ERROR: Could not find Luaurc at {:#?}", luaurc_path.normalize()))
        }
    }

    match scan_for_luaurc(input_dir)
        .or_else(||
            scan_for_luaurc(&input_dir.join("../").normalize())
        )
        {
...
fn startup_message(
    ...
    let to_output_str =
        if let Some(output_dir) = output_dir { &format!(" to {:#?}", output_dir) } else { "" };

    match luaurc_path {
        LuaurcStatus::Some(luaurc_path) =>
            format!("Using Luaurc at {:#?}.\n{} {:#?}{}.", luaurc_path, prefix, input_dir, to_output_str),

        LuaurcStatus::AutoSome(luaurc_path) =>
            format!("Using Luaurc automatically found at {:#?}.\n{} {:#?}{}.", luaurc_path, prefix, input_dir, to_output_str),

        LuaurcStatus::AutoNone =>
            format!("No Luaurc was specified or automatically found.\n{} {:#?}{}.", prefix, input_dir, to_output_str),
    }
}
...
fn canonicalize_input(path: &PathBuf) -> Result<PathBuf, String> {
    match path.canonicalize() {
        Ok(path) => match path.is_dir() {
            true => Ok(path),
            false => Err(format!("ERROR: The specified input {:#?} is not a directory!", path.normalize()))
        },
        Err(_) => Err(format!("ERROR: The specified input directory {:#?} doesn't exist!", path.normalize()))
    }
}
...
fn build(
    ...
    let _ = writeln!(stdout, "{}", startup_message(label, input_dir, output.as_ref(), &luaurc_status));
...
fn main() {
    ...
    match cli.command {
        Commands::Watch { input, output, luaurc_path } => {
            let context = guarded_unwrap!(
                build(input, output, luaurc_path, "RSML CLI is now watching"), return
            );
        ...
        Commands::Build { input, output, luaurc_path } => {
